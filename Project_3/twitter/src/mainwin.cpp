...
int main(int argc, char* argv[])
{
  ...
  // use a hash map to store all hashtags information
  map<string, vector<Tweet*> > hash;
  ...
  while(getline(iFile, tweetLine)) {
    ...
    /*
     * check mention tweet and if is the case
    ...
    /*
     * check whether this tweet has any #
     * if it has #, the for all #
     * first check whether that hash string is already in the map
     * if in the map, just insert that tweet
     * if not in the map, first create that has string and then insert tweet into that string
     */
    Set<string> hashStringSet = newTweet->getHashTags();
    for(Set<string>::iterator it = hashStringSet.begin();
	it != hashStringSet.end();
	++it) {
      string hashString = *it;
      // if that hash string has already been in the hash map, just insert that tweet
      if(hash.find(hashString)!=hash.end() ) {
	hash[hashString].push_back(newTweet);
      }
      // if that hash string hasn't been in the hash map, create that hast string 
      else {
	vector<Tweet*> theTweet;
	hash[hashString] = theTweet;
	hash[hashString].push_back(newTweet);
      }
    }
  }
